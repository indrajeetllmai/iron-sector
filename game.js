...
function selectMap(index){if(party.connected&&party.role==='guest'&&!netApplying)return false;if(state!=='menu'||!loadMap(index))return false;maps.forEach((m,i)=>{$('map-'+i).setAttribute('aria-pressed',String(i===index));$('map-'+i).classList.toggle('selected',i===index)});const m=maps[index];$('map-select').value=String(index);$('map-description').textContent=m.description;$('operation').textContent='OPERATION 0'+(index+1)+' / '+m.name.toUpperCase();$('map-tag').textContent=m.tag;$('hud-map').textContent=$('overlay-eyebrow').textContent='OPERATION '+m.name.toUpperCase();$('deploy-label').textContent='DEPLOY TO '+m.name.toUpperCase();if(!netApplying){sendLobby();lobby()}return true}
loadMap(0);
...
let coopRound=false,roundId='',remoteActor=null,remoteAmmo=[],remoteReserve=[],remoteReload=0,remoteCooldown=0,remoteWeapon=0,remoteKills=0,remoteHits=0,remoteShots=0,remotePoseTime=0,remoteShotSequence=0,guestShotSequence=0,netSequence=0,lastSnapshot=0,netEvents=[],netClock=0,netApplying=false,poseSequence=0,remotePoseSequence=0,pendingPoses=[],pendingShots=[];
const party=new window.IronSquad({status:lobby,message:squadMessage,left:squadLeft,joined:()=>{sendLobby();if(party.role==='guest')party.send({type:'request-lobby'})}});
...
function checkRound(){if(isGuest()||state!=='playing')return;if(teamAlive('red')===0)finish(true);else if(teamAlive('blue')===0)finish(false)}
function eliminate(b,source){if(!b.alive)return;b.health=0;b.alive=false;b.death=.001;b.marker.visible=false;const line=document.createElement('div');line.textContent=source+' / '+b.name;if(isHost())netEvents.push({t:'kill',text:line.textContent});$('killfeed').prepend(line);while($('killfeed').children.length>3)$('killfeed').lastElementChild.remove();checkRound()}
...
function tracer(from,to,color='#e2d4a0'){if(isHost()&&netEvents.length<80)netEvents.push({t:'trace',from:from.toArray(),to:to.toArray(),color});const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints([from,to]),new THREE.LineBasicMaterial({color,transparent:true,opacity:.6}));scene.add(line);effects.push({mesh:line,life:.055,max:.055})}
...
function pause(fromPeer=false){if(!fromPeer&&coopRound)party.send({type:'pause',round:roundId});if(state!=='playing')return;state='paused';trigger=false;aim=false;keys.clear();arrowPressTime.clear();arrowReleaseTime.clear();resetTouch();updateHUD();document.exitPointerLock?.();$('overlay').hidden=false;$('overlay-title').textContent='MISSION PAUSED';$('overlay-copy').textContent='Take a breath. '+maps[mapIndex].name+' can wait.';$('summary').innerHTML='';$('resume').hidden=false;$('restart').hidden=isGuest()}
function resume(fromPeer=false){if(!fromPeer&&coopRound)party.send({type:'resume',round:roundId});state='playing';$('overlay').hidden=true;lock()}
function finish(won,fromPeer=false){state=won?'won':'lost';trigger=false;aim=false;keys.clear();arrowPressTime.clear();arrowReleaseTime.clear();resetTouch();document.exitPointerLock?.();$('overlay').hidden=false;$('overlay-title').textContent=won?'BLUE TEAM WINS':'RED TEAM WINS';$('overlay-copy').textContent=won?'Your squad cleared '+maps[mapIndex].name+'. Ready for another round?':'Your squad was eliminated. Regroup and try another approach.';$('resume').hidden=true;$('restart').hidden=isGuest();$('summary').innerHTML=`<div>${kills}<small>YOUR ELIMINATIONS</small></div><div>${Math.floor(elapsed/60)}:${String(Math.floor(elapsed%60)).padStart(2,'0')}<small>ROUND TIME</small></div><div>${totalShots?Math.round(totalHits/totalShots*100):0}%<small>ACCURACY</small></div>`}
function back(fromPeer=false){if(coopRound&&!fromPeer){party.send({type:'back',round:roundId});if(party.role==='guest')party.leave(false)}coopRound=false;party.busy=false;party.ready=false;remoteActor=null;state='menu';$('loadout').scrollTop=0;$('menu').scrollTop=0;$('menu').hidden=false;$('hud').hidden=true;$('overlay').hidden=true;document.body.classList.remove('playing','aiming');document.exitPointerLock?.();trigger=false;aim=false;keys.clear();arrowPressTime.clear();arrowReleaseTime.clear();gun.visible=false;for(const b of bots)scene.remove(b.group);bots.length=0;lobby()}
...
function botAI(dt){
 ...
 for(const b of bots){
  ...
  const visible=opponents.filter(o=>p.distanceTo(o.position)<26&&clearSight(eye,new THREE.Vector3(o.position.x,o.isPlayer?player.y-.15:o.bot.remote?(o.bot.eyeY||1.7)-.15:1.45,o.position.z))).sort((a,c)=>p.distanceToSquared(a.position)-p.distanceToSquared(c.position));
  ...
  if(target){
   ...
   if(b.fireTimer<=0){
    ...
    tracer(start,new THREE.Vector3(target.position.x,target.isPlayer?player.y-.25:target.bot.remote?(target.bot.eyeY||1.7)-.25:1.3,target.position.z),b.team==='blue'?'#92dfff':'#eed0a3');sound('enemy');
    ...
    if(random()<chance){
     ...
     }else{target.bot.health-=target.bot.remote?((difficulty==='recruit'?4:6)+random()*3)*scale:14+random()*9;target.bot.lastDamage=elapsed;if(target.bot.health<=0)eliminate(target.bot,b.name)}
...
$('deploy').onclick=()=>start();$('pause').onclick=()=>pause();$('resume').onclick=()=>resume();$('restart').onclick=()=>{if(party.connected&&party.role==='guest'){message('HOST STARTS THE NEXT ROUND',2);return}start()};$('back').onclick=()=>back();
...
for(let i=effects.length-1;i>=0;i--){const e=effects[i];e.life-=dt;e.mesh.material.opacity=Math.max(0,e.life/e.max);if(e.life<=0){scene.remove(e.mesh);e.mesh.geometry.dispose();e.mesh.material.dispose();effects.splice(i,1)}}networkTick(dt);renderer.render(scene,camera)}
requestAnimationFrame(frame);$('loading').hidden=true;
// Two-player co-op. The host runs opponents and confirms damage; the guest predicts movement.
function isGuest(){return coopRound&&party.connected&&party.role==='guest'}
function isHost(){return coopRound&&party.connected&&party.role==='host'}
function lobby(){
 const guest=party.connected&&party.role==='guest';
 $('party-status').textContent=party.detail||'Play solo or invite one online friend.';$('party-count').textContent=party.connected?'2 / 2 PLAYERS':'1 / 2 PLAYERS';
 $('party-connected').hidden=!party.connected;$('party-roster').textContent='YOU + '+party.remoteName.toUpperCase()+' · ONLINE';
 $('party-link-panel').hidden=!party.link||party.role!=='host';$('party-link').value=party.link;
 $('invite-friend').disabled=party.connected||['connecting','joining'].includes(party.status);$('join-friend').disabled=party.connected; $('accept-invite').disabled=party.connected||['connecting','joining'].includes(party.status);$('player-name').disabled=party.connected||['connecting','joining'].includes(party.status);if(party.connected)$('party-join-panel').hidden=true;
 $('leave-squad').hidden=!party.role;$('ready-friend').hidden=!guest;$('ready-friend').textContent=party.ready?'READY ✓':'I’M READY';$('ready-friend').setAttribute('aria-pressed',String(party.ready));
 $('party-guide').textContent=guest?'Choose your weapon, press READY, and wait for the host.':party.ready?'Friend ready. Choose a map and deploy together.':'Waiting for your friend to press READY.';
 $('deploy').disabled=guest||party.connected&&!party.ready;
 $('deploy-label').textContent=guest?'WAITING FOR HOST':party.connected?(party.ready?'DEPLOY TOGETHER':'WAITING FOR FRIEND'):'DEPLOY TO '+maps[mapIndex].name.toUpperCase();
 maps.forEach((m,i)=>$('map-'+i).disabled=guest);$('difficulty').disabled=guest;
 $('network-status').textContent=coopRound&&party.connected?'DUO · '+party.remoteName.toUpperCase()+' ONLINE':'';
}
function sendLobby(){if(party.connected&&party.role==='host')party.send({type:'lobby',map:mapIndex,difficulty:$('difficulty').value})}
function renameActor(b,name){b.name=name;b.group.remove(b.marker);b.marker.material.map?.dispose();b.marker.material.dispose();b.marker=teamMarker(name,'blue');b.group.add(b.marker)}
function configureRemote(){remoteActor=bots.find(b=>b.team==='blue'&&b.index===0);if(!remoteActor)return;remoteActor.remote=true;renameActor(remoteActor,party.remoteName.toUpperCase());remoteActor.lastDamage=-100;if(isGuest()){remoteActor.group.position.set(0,0,28);player.set(-12,1.7,26)}remoteAmmo=weapons.map(w=>w.mag);remoteReserve=weapons.map(w=>w.mag*6);remoteReload=0;remoteCooldown=0;remoteKills=0;remoteHits=0;remoteShots=0;remoteWeapon=0;remotePoseTime=elapsed;remoteShotSequence=0;netSequence=0;netEvents=[];netClock=0;poseSequence=0;remotePoseSequence=0;pendingPoses=[];pendingShots=[]}
function start(){
 if(party.connected&&party.role==='guest')return;
 if(party.connected&&!party.ready){$('party-status').textContent='Wait for your friend to press READY.';return}
 coopRound=party.connected;roundId=coopRound?crypto.randomUUID():'';startRound();party.busy=true;
 if(coopRound){configureRemote();party.send({type:'start',round:roundId,map:mapIndex,difficulty:$('difficulty').value});sendSnapshot()}
 lobby();
}
function squadLeft(previousRole=party.role){
 const guest=coopRound&&previousRole==='guest';coopRound=false;party.busy=false;
 if(remoteActor){if(guest){back(true)}else{remoteActor.remote=false;remoteActor.group.scale.y=1;remoteActor.group.position.y=0;renameActor(remoteActor,'ATLAS');message('FRIEND LEFT · ATLAS TAKES OVER',4)}}
 lobby();
}
function pauseTogether(){party.send({type:'pause',round:roundId});pause(true)}
function sendSnapshot(){
 if(!isHost()||!remoteActor)return;
 const actors=bots.filter(b=>b!==remoteActor).map(b=>({id:b.team+b.index,x:b.group.position.x,y:b.group.position.y,z:b.group.position.z,r:b.group.rotation.y,h:b.health,a:b.alive,sy:b.group.scale.y}));
 actors.push({id:'blue0',x:player.x,y:feetY,z:player.z,r:yaw,h:health,a:health>0,sy:eyeHeight/1.7});
 party.send({type:'snapshot',round:roundId,seq:++netSequence,state,elapsed,actors,self:{pose:remotePoseSequence,x:remoteActor.group.position.x,z:remoteActor.group.position.z,shot:remoteShotSequence,h:remoteActor.health,a:remoteActor.alive,k:remoteKills,hits:remoteHits,shots:remoteShots,ammo:remoteAmmo,reserve:remoteReserve,reload:remoteReload,w:remoteWeapon},events:netEvents.splice(0,80)});
}
function validPose(d){return ['x','y','z','yaw','pitch','feet'].every(k=>Number.isFinite(d[k]))&&Math.abs(d.x)<=30.2&&Math.abs(d.z)<=34.2&&d.feet>=0&&d.feet<15&&d.y>=d.feet+.5&&d.y<=d.feet+2&&Math.abs(d.pitch)<=1.5&&Math.abs(d.yaw)<1e6}
function receivePose(d){
 if(!isHost()||state!=='playing'||!remoteActor?.alive||!validPose(d)||!Number.isInteger(d.seq)||d.seq<=remotePoseSequence)return;remotePoseSequence=d.seq;
 const p=remoteActor.group.position,dx=d.x-p.x,dz=d.z-p.z,dist=Math.hypot(dx,dz),budget=Math.min(.3,Math.max(.05,elapsed-remotePoseTime))*9.5+.4;
 if(dist<=budget){const steps=Math.max(1,Math.ceil(dist/.25));for(let i=0;i<steps;i++)move(p,dx/steps,dz/steps,.38,d.feet);p.y=d.feet;remoteActor.eyeY=d.y;remoteActor.group.rotation.y=d.yaw;remoteActor.posePitch=d.pitch;remotePoseTime=elapsed;remoteActor.group.scale.y=Math.max(.4,(d.y-d.feet)/1.7)}
 if(Number.isInteger(d.w)&&weapons[d.w]&&d.w!==remoteWeapon){remoteWeapon=d.w;remoteReload=0}
}
function remoteFire(d){
 if(!isHost()||state!=='playing'||elapsed<3||!remoteActor?.alive||!Number.isInteger(d.seq)||d.seq<=remoteShotSequence)return;
 remoteShotSequence=d.seq;if(remoteCooldown>0||remoteReload>0||!remoteAmmo[remoteWeapon])return;
 if(!Number.isFinite(d.yaw)||!Number.isFinite(d.pitch)||Math.abs(d.pitch)>1.5)return;
 const w=weapons[remoteWeapon];remoteAmmo[remoteWeapon]--;remoteCooldown=w.rate;remoteShots++;
 const origin=remoteActor.group.position.clone();origin.y=remoteActor.eyeY||1.7;
 const direction=new THREE.Vector3(0,0,-1).applyEuler(new THREE.Euler(d.pitch,d.yaw,0,'YXZ'));
 let hitOpponent=false;
 scene.updateMatrixWorld(true);
 for(let pellet=0;pellet<(w.pellets||1);pellet++){
  const dir=direction.clone();const spread=w.spread*(d.aim===true?.22:1);dir.x+=(random()-.5)*spread;dir.y+=(random()-.5)*spread;dir.z+=(random()-.5)*spread;dir.normalize();ray.set(origin,dir);ray.far=100;
  const hits=ray.intersectObjects(walls.concat(bots.filter(b=>b!==remoteActor&&b.alive).map(b=>b.group)),true);let end=origin.clone().addScaledVector(dir,90);
  // The local host also blocks friendly shots even though its body is a first-person camera.
  const toward=player.clone().sub(origin),along=toward.dot(dir),nearHost=health>0&&along>0&&toward.clone().addScaledVector(dir,-along).length()<.7;
  if(nearHost&&(!hits[0]||along<hits[0].distance)){end=origin.clone().addScaledVector(dir,along)}
  else if(hits.length){const h=hits[0];end=h.point;const b=h.object.userData.bot;if(b?.alive&&b.team==='red'){hitOpponent=true;const head=!!h.object.userData.head;const falloff=THREE.MathUtils.clamp(1-Math.max(0,h.distance-(w.pellets?5:18))*(w.falloff||.008),w.minRange||.7,1);b.health-=w.damage*(head?2.5:1)*falloff;b.alert=15;netEvents.push({t:'hit',head});if(b.health<=0){remoteKills++;eliminate(b,party.remoteName.toUpperCase())}impact(h.point,'#c2cda3')}else if(!b)impact(h.point)}tracer(origin,end,'#92dfff');
 }
 if(hitOpponent)remoteHits++;sound('enemy');
}
function applySnapshot(d){
 if(!isGuest()||d.round!==roundId||!Number.isInteger(d.seq)||d.seq<=lastSnapshot||!Array.isArray(d.actors)||d.actors.length!==9||!d.self)return;
 lastSnapshot=d.seq;const self=d.self;const acknowledged=pendingPoses.find(p=>p.seq===self.pose);if(acknowledged&&Number.isFinite(self.x)&&Number.isFinite(self.z)){const dx=self.x-acknowledged.x,dz=self.z-acknowledged.z;if(Math.hypot(dx,dz)>.35){player.x+=dx;player.z+=dz}pendingPoses=pendingPoses.filter(p=>p.seq>self.pose)}pendingShots=pendingShots.filter(p=>p.seq>self.shot);if(!Number.isFinite(self.h)||!Number.isFinite(d.elapsed))return;
 for(const a of d.actors){if(![a.x,a.y,a.z,a.r,a.h].every(Number.isFinite))continue;const b=bots.find(b=>b.team+b.index===a.id);if(!b)continue;b.group.position.set(a.x,a.y,a.z);b.group.rotation.y=a.r;b.group.scale.y=Number.isFinite(a.sy)?THREE.MathUtils.clamp(a.sy,.4,1.2):1;b.health=a.h;b.alive=!!a.a;b.group.rotation.z=b.alive?0:1.5;b.marker.visible=b.alive&&(b.team==='blue'||b.group.position.distanceTo(player)<24&&clearSight(player,b.group.position.clone().add(new THREE.Vector3(0,1.5,0))))}
 if(self.h<health){damageFade=.5;sound('hurt')}health=Math.max(0,Math.min(100,self.h));kills=self.k||0;totalHits=self.hits||0;totalShots=self.shots||0;elapsed=d.elapsed;
 if(Array.isArray(self.ammo)&&self.ammo.length===weapons.length)ammo=self.ammo.map((n,i)=>Math.max(0,Math.min(weapons[i].mag,(Number(n)||0)-pendingShots.filter(p=>p.w===i).length)));
 if(Array.isArray(self.reserve)&&self.reserve.length===weapons.length)reserve=self.reserve.map(n=>Math.max(0,Number(n)||0));reloadTimer=Math.max(0,Number(self.reload)||0);
 document.body.classList.toggle('spectating',health<=0);$('crosshair').hidden=health<=0;
 if(health<=0){trigger=false;keys.clear();resetTouch()}
 for(const e of Array.isArray(d.events)?d.events.slice(0,80):[]){if(e.t==='trace'&&Array.isArray(e.from)&&Array.isArray(e.to)&&e.from.concat(e.to).length===6&&e.from.concat(e.to).every(Number.isFinite))tracer(new THREE.Vector3(...e.from),new THREE.Vector3(...e.to),e.color);if(e.t==='kill'){const line=document.createElement('div');line.textContent=String(e.text).slice(0,80);$('killfeed').prepend(line);while($('killfeed').children.length>3)$('killfeed').lastElementChild.remove()}if(e.t==='hit'){hitTime=.14;$('hitmarker').style.color=e.head?'#d3e6a0':'#fff';sound('hit')}}
 if(d.state==='won'||d.state==='lost'){if(state!==d.state)finish(d.state==='won',true)}else if(d.state==='paused'&&state==='playing')pause(true);else if(d.state==='playing'&&state==='paused')resume(true);
 updateHUD();
}
function squadMessage(d){
 if(typeof d.type!=='string')return;
 if(d.type==='lobby'&&party.role==='guest'&&state==='menu'){netApplying=true;if(Number.isInteger(d.map)&&maps[d.map])selectMap(d.map);if(['recruit','regular','veteran'].includes(d.difficulty))$('difficulty').value=d.difficulty;netApplying=false;lobby()}
 if(d.type==='ready'&&party.role==='host'){party.ready=d.ready===true;lobby()}
 if(d.type==='request-lobby')sendLobby();
 if(d.type==='start'&&party.role==='guest'&&typeof d.round==='string'&&Number.isInteger(d.map)&&maps[d.map]){netApplying=true;state='menu';selectMap(d.map);netApplying=false;$('difficulty').value=['recruit','regular','veteran'].includes(d.difficulty)?d.difficulty:'regular';coopRound=true;roundId=d.round;lastSnapshot=0;guestShotSequence=0;startRound();configureRemote();party.busy=true;lobby();sendPose()}
 if(d.type==='back'&&party.role==='guest'){back(true);party.ready=false;lobby()}
 if(d.round!==roundId||!coopRound)return;
 if(d.type==='pose')receivePose(d);
 if(d.type==='shot')remoteFire(d);
 if(d.type==='reload'&&isHost()&&state==='playing'&&remoteActor?.alive&&!remoteReload&&remoteAmmo[remoteWeapon]<weapons[remoteWeapon].mag&&remoteReserve[remoteWeapon]>0)remoteReload=weapons[remoteWeapon].reload;
 if(d.type==='snapshot')applySnapshot(d);
 if(d.type==='pause'&&state==='playing'){pause(true);if(isHost())sendSnapshot()}
 if(d.type==='resume'&&state==='paused'){resume(true);if(isHost())sendSnapshot()}
}
function sendPose(){const pose={...localPose(),seq:++poseSequence};pendingPoses.push(pose);if(pendingPoses.length>120)pendingPoses.shift();party.send({type:'pose',round:roundId,...pose})}
function localPose(){return {x:player.x,y:player.y,z:player.z,feet:feetY,yaw,pitch,w:weaponIndex}}
function networkTick(dt){
 if(!party.connected||!coopRound)return;
 if(isHost()&&state==='playing'){
  remoteCooldown=Math.max(0,remoteCooldown-dt);
  if(remoteReload>0){remoteReload-=dt;if(remoteReload<=0){remoteReload=0;const take=Math.min(weapons[remoteWeapon].mag-remoteAmmo[remoteWeapon],remoteReserve[remoteWeapon]);remoteAmmo[remoteWeapon]+=take;remoteReserve[remoteWeapon]-=take}}
  if(remoteActor?.alive&&elapsed-remoteActor.lastDamage>8)remoteActor.health=Math.min(100,remoteActor.health+dt*5);
 }
 netClock+=dt;if(netClock<.05)return;netClock=0;
 if(isHost())sendSnapshot();else if(isGuest()&&state==='playing')sendPose();
}

$('invite-friend').onclick=()=>{if(state==='menu')party.host($('player-name').value)};
$('join-friend').onclick=()=>{$('party-join-panel').hidden=!$('party-join-panel').hidden};
$('accept-invite').onclick=()=>{if(state==='menu'){audio();party.join($('join-link').value,$('player-name').value)}};
$('copy-invite').onclick=async()=>{try{await navigator.clipboard.writeText(party.link);$('party-status').textContent='Link copied. Send it to one friend.'}catch{$('party-link').focus();$('party-link').select();$('party-status').textContent='Select and copy the invite link above.'}};
$('ready-friend').onclick=()=>{audio();party.ready=!party.ready;party.send({type:'ready',ready:party.ready});lobby()};
$('leave-squad').onclick=()=>{const wasGuest=isGuest();party.leave();if(wasGuest&&state!=='menu')back(true);lobby()};
$('difficulty').onchange=sendLobby;
if(location.hash.startsWith('#squad=')){$('party-join-panel').hidden=false;$('join-link').value=location.href;$('party-status').textContent='You have a squad invite. Enter your name and join your friend.'}
lobby();
addEventListener('beforeunload',()=>party.leave(false));
// Optional browser agent hooks expose only normal menu actions and game status.
const modelContext=document.modelContext;
if(modelContext?.registerTool){
  const tools=[{
    name:'choose_loadout',description:'Choose a weapon and mission settings. In a duo, only the host chooses map and difficulty.',
    ...
    execute:async(input)=>{
      ...
      if(party.connected&&party.role==='guest'&&(input.map||input.difficulty))throw new Error('The host chooses map and difficulty.');
      if(state!=='menu')throw new Error('Return to the loadout screen first.');
      if(input.map)selectMap(maps.findIndex(m=>m.name===input.map));setWeapon(index,false);if(input.difficulty)$('difficulty').value=input.difficulty;sendLobby();
    ...
    execute:async()=>({state,squad:{role:party.role,connected:party.connected,players:party.connected?2:1,status:party.status,friend:party.connected?party.remoteName:null},map:maps[mapIndex].name,weapon:weapons[weaponIndex].name,health:Math.ceil(health),position:{x:Math.round(player.x*100)/100,z:Math.round(player.z*100)/100},eliminations:kills,opponentsRemaining:teamAlive('red'),blueAlive:teamAlive('blue'),aiming:aim,crouched,jumping:!grounded,sliding:slideTimer>0,controls:touch?'touch':'desktop',difficulty:$('difficulty').value,ammo:ammo[weaponIndex],teams:{blue:bots.filter(b=>b.team==='blue').map(b=>({name:b.name,alive:b.alive})),red:bots.filter(b=>b.team==='red').map(b=>({name:b.name,alive:b.alive}))}})
  }];
